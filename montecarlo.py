...
# Personal data
starting_capital = inputs['starting_capital']    # Starting capital
retirement_age = inputs['retirement_age']        # Retirement age
samples = inputs['samples']                      # Number of samples for the Montecarlo simulation
dob = dt.datetime.strptime(inputs['dob'], '%Y-%m-%d')
age = age(dob)
max_age = inputs['max_age']             # The age you expect to die

# Allocation
current_stocks = 0.72                   # Starting stocks percentage in portfolio
current_bonds = (1 - current_stocks)    # Starting bonds percentage in portfolio
stocks_to_bonds = 0.01                  # Because you are not getting any younger, each year the portfolio is rebalanced towards bonds by this percentage
min_stocks = 0.4                        # Minimum percentage of stocks in portfolio
max_bonds  = (1.0 - min_stocks)         # Calculated maximum percentage of bonds

# Returns and volatility
mu_stocks = 0.04 # Stocks return after inflation
mu_bonds = 0.005 # Bonds return after inflation
vol_stocks = 0.25 # Stocks volatility (standard deviation)
vol_bonds = 0.05 # Bonds volatility (standard deviation)


# Contribution before retirement
personal_contribution = 2000.0    # Monthly personal contribution without government topup (Net). This does not include work match
work_contribution     = 263.0 * 2 # Monthly work contribution without government topup. Multiply by 2 if you match the company contribution
annual_contribution = 12.0 * personal_contribution + 12 * work_contribution 
contribution_growth = 0.02 # Because of payrise for example
gov_topup = 0.25         # Higher tax rate. This is valid only in UK and as of 2021
contribution_during_retirment = 0.0     # If for example you have other sources of income


# Expenses During retirement
annual_expenses = 12 * 2000.0           # Net expenses once you retire.
tax_free_allowance = 0.25               # Valid in UK
state_pension = 9100.0                  # After 66 as of 2021
state_pension_age = 66                  # As of 2021    
...
bands = inputs['tax_bands'] 
rates = inputs['tax_rates']
...
annual_withdrawal = net_to_gross(taxable_annual_expenses, bands, rates) + tax_free_allowance * annual_expenses # Total gross withdrawal
 

living_years = max_age - age                    # Number of years left
working_years = retirement_age - age            # Number of working years based on your expected retiremen age
state_pension_years = state_pension_age - age   # Number of years before state pension
print("age:                     " + str(age))
...
for i in range(samples):
    ...
    annual_returns_stocks = np.random.normal(mu_stocks, vol_stocks, living_years)
    ...
    for year, (annual_return_stocks, annual_return_bonds) in enumerate(zip(annual_returns_stocks, annual_returns_bonds)):
        ...
        if year <= working_years: # Before retirement
            ...
            final_annual_value = stocks * starting_annual_value * (annual_return_stocks + 1.0) + \
                                 bonds  * starting_annual_value * (annual_return_bonds   + 1.0)     # Change of previous year capital        
            annual_values.append(final_annual_value + annual_investment)                            # Additional investemnt at the end of year 
            ...
            if sustainable_withdrawal_rate: # To be reviewed
                if swr_percentage * annual_values[-1] > annual_withdrawal:    
                    ...
                    actual_annual_withdrawal = swr_percentage * annual_values[-1]
            else:
                if year <= state_pension_years:
                    actual_annual_withdrawal = annual_withdrawal                  # I need to withdraw all expected expesnes
                else:
                    actual_annual_withdrawal = annual_withdrawal - state_pension  # Little help from state pension
            annual_withdrawals.append(actual_annual_withdrawal)
            
            starting_annual_value = annual_values[-1] - actual_annual_withdrawal
            final_annual_value = stocks * starting_annual_value * (annual_return_stocks + 1.0) + \
                                 bonds  * starting_annual_value * (annual_return_bonds  + 1.0)
            annual_values.append(final_annual_value)
        
        if year <= working_years:                               # Before retirement
            annual_investment *= (1.0 + contribution_growth)    # Keep investing until retiremnt
        else:                                                   # After retirement - No growth
            annual_investment = contribution_during_retirment   # Assume it will not increase. Safely I assume 0.0
            

        stocks = max(min_stocks, stocks - stocks_to_bonds) # Limit stocks contribution to min_stocks% 
        bonds  = min(max_bonds, bonds  + stocks_to_bonds) # Limit bonds contribution to max_bonds%
    
    min_annual_withdrawal = min(annual_withdrawals) # This is only valid if sustainable_withdrawal_rate == True
